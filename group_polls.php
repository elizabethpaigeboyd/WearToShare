...
if (!$user_id || !$group_id) {
    echo "<p>Error: Missing user or group info.</p>";
    return;
}
...
if (isset($_POST['create_poll']) && isset($_POST['question'], $_POST['image_a'], $_POST['image_b'])) {
    ...
    try {
        $insertPoll = $conn->prepare("INSERT INTO polls (question, created_by_user_id, group_id, event_id) VALUES (?, ?, ?, NULL)");
        $insertPoll->bind_param("sii", $question, $user_id, $group_id);
        $insertPoll->execute();
...
// === Display all polls for the group ===
$stmt = $conn->prepare("
    SELECT p.*, u.username 
    FROM polls p 
...
if ($pollResult->num_rows > 0) {
    ...
    echo "<p>No polls yet for this group.</p>";
}
?>
